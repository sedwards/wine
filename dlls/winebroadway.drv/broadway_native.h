...
struct broadway_native_base_t
{
    int magic;
    int version;
    void *reserved[4];
    void (*incRef)(struct broadway_native_base_t *base);
    void (*decRef)(struct broadway_native_base_t *base);
};

typedef struct broadway_native_rect_t
{
    int32_t left;
    int32_t top;
    int32_t right;
    int32_t bottom;
} broadway_native_rect_t;

struct ANativeWindowBuffer
{
    struct broadway_native_base_t common;
    int width;
    int height;
    int stride;
    int format;
...
struct gralloc_module_t
{
    ...
    int (*lock_ycbcr)(struct gralloc_module_t const *module, buffer_handle_t handle, int usage, int l, int t, int w, int h, struct broadway_ycbcr *ycbcr);
    void *reserved_proc[6];
};

#define BROADWAY_NATIVE_MAKE_CONSTANT(a,b,c,d) \
    (((unsigned)(a)<<24)|((unsigned)(b)<<16)|((unsigned)(c)<<8)|(unsigned)(d))

#define BROADWAY_NATIVE_WINDOW_MAGIC \
    BROADWAY_NATIVE_MAKE_CONSTANT('_','w','n','d')

#define BROADWAY_NATIVE_BUFFER_MAGIC \
    BROADWAY_NATIVE_MAKE_CONSTANT('_','b','f','r')
...
#endif /* __WINE_BROADWAY_NATIVE_H */
