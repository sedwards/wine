...
#define BROADWAY_IOCTL(n) CTL_CODE(BROADWAYCONTROLTYPE, n, METHOD_BUFFERED, FILE_READ_ACCESS)
...
struct ioctl_broadway_create_window
{
...
struct ioctl_broadway_destroy_window
{
    struct ioctl_header hdr;
};

struct ioctl_broadway_window_pos_changed
{
    struct ioctl_header hdr;
    RECT                window_rect;
    RECT                client_rect;
...
struct ioctl_broadway_dequeueBuffer
{
    struct ioctl_header hdr;
    int                 win32;
    int                 width;
...
struct ioctl_broadway_queueBuffer
{
    struct ioctl_header hdr;
    int                 buffer_id;
    int                 generation;
...
struct ioctl_broadway_cancelBuffer
{
    struct ioctl_header hdr;
    int                 buffer_id;
    int                 generation;
...
struct ioctl_broadway_set_swap_interval
{
    struct ioctl_header hdr;
    int                 interval;
};

struct ioctl_broadway_set_window_parent
{
    struct ioctl_header hdr;
    int                 parent;
    float               scale;
...
struct ioctl_broadway_set_capture
{
    struct ioctl_header hdr;
};

struct ioctl_broadway_set_cursor
{
    struct ioctl_header hdr;
    int                 id;
    int                 width;
...
NTSTATUS broadway_register_window( void *arg )
{
    ...
    unwrap_java_call();
    NtUserPostMessage( hwnd, WM_BROADWAY_REFRESH, opengl, 0 );
    TRACE( "%p -> %p win %p\n", hwnd, data, win );
    return 0;
}
...
static NTSTATUS broadway_error_to_status( int err )
{
    switch (err)
    {
...
static int status_to_broadway_error( unsigned int status )
{
    switch (status)
    {
...
static NTSTATUS dequeueBuffer_ioctl( void *data, DWORD in_size, DWORD out_size, ULONG_PTR *ret_size )
{
    ...
    if (in_size < offsetof( struct ioctl_broadway_dequeueBuffer, native_handle ))
    ...
    *ret_size = offsetof( struct ioctl_broadway_dequeueBuffer, native_handle );
    wrap_java_call();
...
static NTSTATUS queueBuffer_ioctl( void *data, DWORD in_size, DWORD out_size, ULONG_PTR *ret_size )
{
    ...
    if (win_data->mappings[res->buffer_id])
    {
        ...
        if (ret) return broadway_error_to_status( ret );
...
{
    struct ioctl_broadway_query *res = data;
    struct ANativeWindow *parent;
    struct native_win_data *win_data;
    int ret;
...
{
    struct ioctl_broadway_perform *res = data;
    struct ANativeWindow *parent;
    struct native_win_data *win_data;
    int ret = -ENOENT;
...
{
    struct ioctl_broadway_set_swap_interval *res = data;
    struct ANativeWindow *parent;
    struct native_win_data *win_data;
    int ret;
...
{
    struct ioctl_broadway_set_capture *res = data;
...
static NTSTATUS setCursor_ioctl( void *data, DWORD in_size, DWORD out_size, ULONG_PTR *ret_size )
{
    ...
    jobject object;
    int size;
    struct ioctl_broadway_set_cursor *res = data;

    if (in_size < offsetof( struct ioctl_broadway_set_cursor, bits )) return STATUS_INVALID_PARAMETER;

    if (res->width < 0 || res->height < 0 || res->width > 256 || res->height > 256)
        return STATUS_INVALID_PARAMETER;

    size = res->width * res->height;
    if (in_size != offsetof( struct ioctl_broadway_set_cursor, bits[size] ))
        return STATUS_INVALID_PARAMETER;
...
NTSTATUS broadway_dispatch_ioctl( void *arg )
{
    struct ioctl_params *params = arg;
    IRP *irp = params->irp;
    IO_STACK_LOCATION *irpsp = IoGetCurrentIrpStackLocation( irp );
    DWORD code = (irpsp->Parameters.DeviceIoControl.IoControlCode - BROADWAY_IOCTL(0)) >> 2;
...
void start_broadway_device(void)
{
    void *ret_ptr;
    ULONG ret_len;
...
static int broadway_ioctl( enum broadway_ioctl code, void *in, DWORD in_size, void *out, DWORD *out_size )
{
    ...
    status = NtDeviceIoControlFile( device, NULL, NULL, NULL, &iosb, BROADWAY_IOCTL(code),
    ...
    return status_to_broadway_error( status );
}

static void win_incRef( struct broadway_native_base_t *base )
{
    struct native_win_wrapper *win = (struct native_win_wrapper *)base;
    InterlockedIncrement( &win->ref );
}

static void win_decRef( struct broadway_native_base_t *base )
{
    struct native_win_wrapper *win = (struct native_win_wrapper *)base;
    InterlockedDecrement( &win->ref );
}

static void buffer_incRef( struct broadway_native_base_t *base )
{
    struct native_buffer_wrapper *buffer = (struct native_buffer_wrapper *)base;
    InterlockedIncrement( &buffer->ref );
}

static void buffer_decRef( struct broadway_native_base_t *base )
{
    struct native_buffer_wrapper *buffer = (struct native_buffer_wrapper *)base;
...
static int dequeueBuffer( struct ANativeWindow *window, struct ANativeWindowBuffer **buffer, int *fence )
{
    ...
    struct ioctl_broadway_dequeueBuffer res;
    ...
    res.win32 = use_win32;
    ret = broadway_ioctl( IOCTL_DEQUEUE_BUFFER,
                         &res, offsetof( struct ioctl_broadway_dequeueBuffer, native_handle ),
                         &res, &size );
    if (ret) return ret;

    /* if we received the native handle, this is a new buffer */
    if (size > offsetof( struct ioctl_broadway_dequeueBuffer, native_handle ))
    {
        struct native_buffer_wrapper *buf = calloc( 1, sizeof(*buf) );

        buf->buffer.common.magic   = BROADWAY_NATIVE_BUFFER_MAGIC;
...
static int cancelBuffer( struct ANativeWindow *window, struct ANativeWindowBuffer *buffer, int fence )
{
    ...
    struct ioctl_broadway_cancelBuffer cancel;
    ...
    wait_fence_and_close( fence );
    return broadway_ioctl( IOCTL_CANCEL_BUFFER, &cancel, sizeof(cancel), NULL, NULL );
}
...
static int queueBuffer( struct ANativeWindow *window, struct ANativeWindowBuffer *buffer, int fence )
{
    struct native_win_wrapper *win = (struct native_win_wrapper *)window;
    struct native_buffer_wrapper *buf = (struct native_buffer_wrapper *)buffer;
    struct ioctl_broadway_queueBuffer queue;
    ...
    wait_fence_and_close( fence );
    return broadway_ioctl( IOCTL_QUEUE_BUFFER, &queue, sizeof(queue), NULL, NULL );
}
...
{
    struct native_win_wrapper *win = (struct native_win_wrapper *)window;
    struct ioctl_broadway_set_swap_interval swap;
...
static int query( const ANativeWindow *window, int what, int *value )
{
    ...
    query.what = what;
    ret = broadway_ioctl( IOCTL_QUERY, &query, sizeof(query), &query, &size );
...
static int perform( ANativeWindow *window, int operation, ... )
{
    ...
    struct native_win_wrapper *win = (struct native_win_wrapper *)window;
    struct ioctl_broadway_perform perf;
    va_list args;
    ...
    case NATIVE_WINDOW_SET_CROP:
    {
        broadway_native_rect_t *rect = va_arg( args, broadway_native_rect_t * );
...
struct ANativeWindow *create_ioctl_window( HWND hwnd, BOOL opengl, float scale )
{
    ...
    req.scale = scale;
    broadway_ioctl( IOCTL_CREATE_WINDOW, &req, sizeof(req), NULL, NULL );
...
{
    struct ioctl_broadway_destroy_window req;

    req.hdr.hwnd = HandleToLong( hwnd );
    req.hdr.opengl = opengl;
    broadway_ioctl( IOCTL_DESTROY_WINDOW, &req, sizeof(req), NULL, NULL );
}
...
{
    struct ioctl_broadway_window_pos_changed req;
    ...
    return broadway_ioctl( IOCTL_WINDOW_POS_CHANGED, &req, sizeof(req), NULL, NULL );
}

int ioctl_set_window_parent( HWND hwnd, HWND parent, float scale )
{
    struct ioctl_broadway_set_window_parent req;

    req.hdr.hwnd = HandleToLong( hwnd );
    req.hdr.opengl = FALSE;
    req.parent = get_ioctl_win_parent( parent );
    req.scale = scale;
    return broadway_ioctl( IOCTL_SET_WINDOW_PARENT, &req, sizeof(req), NULL, NULL );
}

int ioctl_set_capture( HWND hwnd )
{
    struct ioctl_broadway_set_capture req;

    req.hdr.hwnd  = HandleToLong( hwnd );
    req.hdr.opengl = FALSE;
    return broadway_ioctl( IOCTL_SET_CAPTURE, &req, sizeof(req), NULL, NULL );
}

int ioctl_set_cursor( int id, int width, int height,
                      int hotspotx, int hotspoty, const unsigned int *bits )
{
    struct ioctl_broadway_set_cursor *req;
    unsigned int size = offsetof( struct ioctl_broadway_set_cursor, bits[width * height] );
    int ret;
    ...
    ret = broadway_ioctl( IOCTL_SET_CURSOR, req, size, NULL, NULL );
    free( req );
    return ret;
}
