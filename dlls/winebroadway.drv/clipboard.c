                               ...
                               unsigned char **data, size_t *size )
{
    ...
    XConvertSelection( display, selection, format->atom, broadwaydrv_atom(SELECTION_DATA), win, CurrentTime );
...
static void *import_compound_text( Atom type, const void *data, size_t size, size_t *ret_size )
{
    ...
    txtprop.nitems = size;
    txtprop.encoding = broadwaydrv_atom(COMPOUND_TEXT);
    txtprop.format = 8;
...
static void *import_text( Atom type, const void *data, size_t size, size_t *ret_size )
{
    ...
    if (type == broadwaydrv_atom(UTF8_STRING)) return import_utf8_string( type, data, size, ret_size );
    if (type == broadwaydrv_atom(COMPOUND_TEXT)) return import_compound_text( type, data, size, ret_size );
...
static void *import_targets( Atom type, const void *data, size_t size, size_t *ret_size )
{
    ...
    if (type != XA_ATOM && type != broadwaydrv_atom(TARGETS)) return 0;
...
static BOOL export_text( Display *display, Window win, Atom prop, Atom target, void *data, size_t size )
{
    return export_utf8_string( display, win, prop, broadwaydrv_atom(UTF8_STRING), data, size );
}
                                  ...
                                  void *data, size_t size )
{
    ...
    if (target == broadwaydrv_atom(COMPOUND_TEXT))
...
static void acquire_selection( Display *display )
{
    ...
    XSetSelectionOwner( display, broadwaydrv_atom(CLIPBOARD), selection_window, CurrentTime );
...
{
    struct clipboard_format *targets = find_x11_format( broadwaydrv_atom(TARGETS) );
    ...
    {
        if ((owner = XGetSelectionOwner( display, broadwaydrv_atom(CLIPBOARD) )))
            current_selection = broadwaydrv_atom(CLIPBOARD);
    }
...
static void xfixes_init(void)
{
    ...
    pXFixesSelectSelectionInput(clipboard_display, import_window, broadwaydrv_atom(CLIPBOARD),
...
BOOL X11DRV_SelectionRequest( HWND hwnd, XEvent *xev )
{
    ...
    if ((event->selection != broadwaydrv_atom(CLIPBOARD)) &&
...
BOOL X11DRV_SelectionClear( HWND hwnd, XEvent *xev )
{
    ...
    if (event->selection != broadwaydrv_atom(CLIPBOARD)) return FALSE;
