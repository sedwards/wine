...
/* X context to associate a struct broadwaydrv_win_data to an hwnd */
...
static void remove_startup_notification(Display *display, Window window)
{
    ...
    xevent.xclient.message_type = broadwaydrv_atom(_NET_STARTUP_INFO_BEGIN);
    ...
    while (srclen > 0)
    {
        ...
        xevent.xclient.message_type = broadwaydrv_atom(_NET_STARTUP_INFO);
    }
}
 ...
 *              alloc_win_data
 */
static struct broadwaydrv_win_data *alloc_win_data( Display *display, HWND hwnd )
{
 ...
 */
static inline BOOL is_window_resizable( struct broadwaydrv_win_data *data, DWORD style )
{
    if (style & WS_THICKFRAME) return TRUE;
 ...
 *              get_mwm_decorations
 */
static unsigned long get_mwm_decorations( struct broadwaydrv_win_data *data,
 ...
 */
static int get_window_attributes( struct broadwaydrv_win_data *data, XSetWindowAttributes *attr )
{
 ...
 */
static void sync_window_style( struct broadwaydrv_win_data *data )
{
 ...
 */
static void sync_window_region( struct broadwaydrv_win_data *data, HRGN win_region )
{
#ifdef HAVE_LIBXSHAPE
                                 ...
                                 COLORREF key, BYTE alpha, DWORD flags )
{
    ...
    if (opacity == 0xffffffff)
        XDeleteProperty( display, win, broadwaydrv_atom(_NET_WM_WINDOW_OPACITY) );
    else
        XChangeProperty( display, win, broadwaydrv_atom(_NET_WM_WINDOW_OPACITY),
...
static void sync_window_text( Display *display, Window win, const WCHAR *text )
{
    ...
    */
    XChangeProperty( display, win, broadwaydrv_atom(_NET_WM_NAME), broadwaydrv_atom(UTF8_STRING),
 ...
 */
static void set_size_hints( struct broadwaydrv_win_data *data, DWORD style )
{
 ...
 *              set_mwm_hints
 */
static void set_mwm_hints( struct broadwaydrv_win_data *data, UINT style, UINT ex_style )
{
    MwmHints mwm_hints;
    ...
    mwm_hints.status = 0;
    XChangeProperty( data->display, data->whole_window, broadwaydrv_atom(_MOTIF_WM_HINTS),
                     broadwaydrv_atom(_MOTIF_WM_HINTS), 32, PropModeReplace,
 ...
 *              set_style_hints
 */
static void set_style_hints( struct broadwaydrv_win_data *data, DWORD style, DWORD ex_style )
{
    ...
    if (((style & WS_POPUP) || (ex_style & WS_EX_DLGMODALFRAME)) && owner)
        window_type = broadwaydrv_atom(_NET_WM_WINDOW_TYPE_DIALOG);
    else
        window_type = broadwaydrv_atom(_NET_WM_WINDOW_TYPE_NORMAL);

    XChangeProperty(data->display, data->whole_window, broadwaydrv_atom(_NET_WM_WINDOW_TYPE),
    ...
    if (data->icon_bits)
        XChangeProperty( data->display, data->whole_window, broadwaydrv_atom(_NET_WM_ICON),
                         XA_CARDINAL, 32, PropModeReplace,
                         (unsigned char *)data->icon_bits, data->icon_size );
    else
        XDeleteProperty( data->display, data->whole_window, broadwaydrv_atom(_NET_WM_ICON) );
...
static void set_initial_wm_hints( Display *display, Window window )
{
    ...
    /* wm protocols */
    i = 0;
    protocols[i++] = broadwaydrv_atom(WM_DELETE_WINDOW);
    protocols[i++] = broadwaydrv_atom(_NET_WM_PING);
    if (use_take_focus) protocols[i++] = broadwaydrv_atom(WM_TAKE_FOCUS);
    XChangeProperty( display, window, broadwaydrv_atom(WM_PROTOCOLS),
    ...
    i = getpid();
    XChangeProperty(display, window, broadwaydrv_atom(_NET_WM_PID),
                    XA_CARDINAL, 32, PropModeReplace, (unsigned char *)&i, 1);

    XChangeProperty( display, window, broadwaydrv_atom(XdndAware),
 ...
 */
static void set_wm_hints( struct broadwaydrv_win_data *data )
{
    DWORD style, ex_style;
...
void update_user_time( Time time )
{
    ...
    {
        last_user_time = time;
        XChangeProperty( gdi_display, user_time_window, broadwaydrv_atom(_NET_WM_USER_TIME),
...
static void update_net_wm_fullscreen_monitors( struct broadwaydrv_win_data *data )
{
    long monitors[4];
    XEvent xev;
    ...
    if (!data->mapped)
    {
        XChangeProperty( data->display, data->whole_window, broadwaydrv_atom(_NET_WM_FULLSCREEN_MONITORS),
    ...
    else
    {
        ...
        xev.xclient.message_type = broadwaydrv_atom(_NET_WM_FULLSCREEN_MONITORS);
 ...
 */
void update_net_wm_states( struct broadwaydrv_win_data *data )
{
    ...
    if (!data->mapped)  /* set the _NET_WM_STATE atom directly */
    {
        ...
        XChangeProperty( data->display, data->whole_window, broadwaydrv_atom(_NET_WM_STATE), XA_ATOM,
    ...
    else  /* ask the window manager to do it for us */
    {
        ...
        xev.xclient.type = ClientMessage;
        xev.xclient.window = data->whole_window;
        xev.xclient.message_type = broadwaydrv_atom(_NET_WM_STATE);
        xev.xclient.serial = 0;
        xev.xclient.display = data->display;
        xev.xclient.send_event = True;
        xev.xclient.format = 32;
 ...
 *     read_net_wm_states
 */
void read_net_wm_states( Display* display, struct broadwaydrv_win_data *data )
{
    Atom type, *state;
    int format;
    ...
    if (!XGetWindowProperty( display, data->whole_window, broadwaydrv_atom(_NET_WM_STATE), 0,
                             ...
                             &remaining, (unsigned char **)&state ))
    {
        if (type == XA_ATOM && format == 32)
        {
            for (i = 0; i < count; i++)
            {
                if (state[i] == broadwaydrv_atom(_NET_WM_STATE_MAXIMIZED_HORZ))
                    maximized_horz = TRUE;
 ...
 *     set_xembed_flags
 */
static void set_xembed_flags( struct broadwaydrv_win_data *data, unsigned long flags )
{
    ...
    info[1] = flags;
    XChangeProperty( data->display, data->whole_window, broadwaydrv_atom(_XEMBED_INFO),
                     broadwaydrv_atom(_XEMBED_INFO), 32, PropModeReplace, (unsigned char*)info, 2 );
}
 ...
 */
void make_window_embedded( struct broadwaydrv_win_data *data )
{
 ...
 *     get_decoration_rect
 */
static void get_decoration_rect( struct broadwaydrv_win_data *data, RECT *rect,
 ...
 */
static void sync_window_position( struct broadwaydrv_win_data *data,
 ...
 */
static void sync_client_position( struct broadwaydrv_win_data *data,
 ...
 */
static void create_whole_window( struct broadwaydrv_win_data *data )
{
    int cx, cy, mask;
    XSetWindowAttributes attr;
    WCHAR text[1024];
 ...
 */
static void destroy_whole_window( struct broadwaydrv_win_data *data, BOOL already_destroyed )
{
 ...
 */
void set_window_visual( struct broadwaydrv_win_data *data, const XVisualInfo *vis, BOOL use_alpha )
{
...
void X11DRV_SetDesktopWindow( HWND hwnd )
{
    ...
    if (!width && !height)  /* not initialized yet */
    {
        ...
        {
            Display *display = broadwaydrv_thread_data()->display;
            TRACE("setting desktop to fullscreen\n");
            XChangeProperty( display, root_window, broadwaydrv_atom(_NET_WM_STATE), XA_ATOM, 32, PropModeReplace,
                             (unsigned char*)&broadwaydrv_atom(_NET_WM_STATE_FULLSCREEN), 1 );
        }
    }
    else
    {
 ...
 */
struct broadwaydrv_win_data *get_win_data( HWND hwnd )
{
    char *data;
    ...
    if (!XFindContext( gdi_display, (XID)hwnd, win_data_context, &data ))
        return (struct broadwaydrv_win_data *)data;
 ...
 */
void release_win_data( struct broadwaydrv_win_data *data )
{
...
NTSTATUS broadwaydrv_systray_init( void *arg )
{
    Display *display;
    ...
    if (DefaultScreen( display ) == 0)
        systray_atom = broadwaydrv_atom(_NET_SYSTEM_TRAY_S0);
    else
    {
...
NTSTATUS broadwaydrv_systray_hide( void *arg )
{
    HWND hwnd = *(HWND*)arg;
    struct broadwaydrv_win_data *data;
...
static void get_systray_visual_info( Display *display, Window systray_window, XVisualInfo *info )
{
    ...
    *info = default_visual;
    if (XGetWindowProperty( display, systray_window, broadwaydrv_atom(_NET_SYSTEM_TRAY_VISUAL), 0,
...
NTSTATUS broadwaydrv_systray_dock( void *arg )
{
    ...
    *params->layered = layered = (visual.depth == 32);
    ...
    ev.xclient.message_type = broadwaydrv_atom( _NET_SYSTEM_TRAY_OPCODE );
...
void X11DRV_WindowPosChanged( HWND hwnd, HWND insert_after, UINT swp_flags,
                              ...
                              struct window_surface *surface )
{
    struct broadwaydrv_thread_data *thread_data;
    struct broadwaydrv_win_data *data;
    ...
    thread_data = broadwaydrv_thread_data();
...
static BOOL hide_icon( struct broadwaydrv_win_data *data )
{
...
static BOOL is_netwm_supported( Display *display, Atom atom )
{
    ...
    if (net_supported_count == -1)
    {
        ...
        if (!XGetWindowProperty( display, DefaultRootWindow(display), broadwaydrv_atom(_NET_SUPPORTED), 0,
...
LRESULT X11DRV_SysCommand( HWND hwnd, WPARAM wparam, LPARAM lparam )
{
    ...
    if (!is_netwm_supported( data->display, broadwaydrv_atom(_NET_WM_MOVERESIZE) ))
    {
...
void X11DRV_FlashWindowEx( FLASHWINFO *pfinfo )
{
    ...
    if (data->mapped)
    {
        ...
        xev.xclient.data.l[1] = broadwaydrv_atom( _NET_WM_STATE_DEMANDS_ATTENTION );
