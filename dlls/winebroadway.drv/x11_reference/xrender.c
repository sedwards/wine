                                   ...
                                   PHYSDEV src_dev, struct bitblt_coords *src, DWORD rop )
{
    ...
    if (physdev_dst->format == WXR_FORMAT_MONO && physdev_src->format != WXR_FORMAT_MONO)
        goto broadwaydrv_fallback;

    /* if not stretching, we only need to handle format conversion */
    if (!stretch && physdev_dst->format == physdev_src->format) goto broadwaydrv_fallback;
...
static HBRUSH xrenderdrv_SelectBrush( PHYSDEV dev, HBRUSH hbrush, const struct brush_pattern *pattern )
{
    ...
    if (!pattern) goto broadwaydrv_fallback;
    if (pattern->info->bmiHeader.biBitCount == 1) goto broadwaydrv_fallback;
    if (physdev->format == WXR_FORMAT_MONO) goto broadwaydrv_fallback;
