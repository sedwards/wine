...
static void update_relative_valuators(XIAnyClassInfo **valuators, int n_valuators)
{
    ...
    for (i = 0; i < n_valuators; i++)
    {
        ...
        if (class->label == broadwaydrv_atom( Rel_X ) ||
            (!class->label && class->number == 0 && class->mode == XIModeRelative))
            thread_data->x_valuator = *class;
        else if (class->label == broadwaydrv_atom( Rel_Y ) ||
...
static void send_mouse_input( HWND hwnd, Window window, unsigned int state, INPUT *input )
{
    ...
    if (hwnd != broadwaydrv_thread_data()->grab_hwnd &&
...
void BROADWAYDRV_SetCursor( HWND hwnd, HCURSOR handle )
{
    ...
    if (clipping_cursor) set_window_cursor( broadwaydrv_thread_data()->clip_window, handle );
}
...
void move_resize_window( HWND hwnd, int dir )
{
    ...
    xev.xclient.message_type = broadwaydrv_atom(_NET_WM_MOVERESIZE);
...
BOOL BROADWAYDRV_EnterNotify( HWND hwnd, XEvent *xev )
{
    ...
    if (hwnd == broadwaydrv_thread_data()->grab_hwnd) return FALSE;
