...
static void update_desktop_fullscreen( unsigned int width, unsigned int height)
{
    ...
    xev.xclient.data.l[1] = broadwaydrv_atom(_NET_WM_STATE_FULLSCREEN);
    ...
    xev.xclient.data.l[1] = broadwaydrv_atom(_NET_WM_STATE_MAXIMIZED_VERT);
    xev.xclient.data.l[2] = broadwaydrv_atom(_NET_WM_STATE_MAXIMIZED_HORZ);
    XSendEvent( display, DefaultRootWindow(display), False,
                SubstructureRedirectMask | SubstructureNotifyMask, &xev );
}
