...
static broadwaydrv_event_handler handlers[MAX_EVENT_HANDLERS] =
{
    NULL,                     /*  0 reserved */
    NULL,                     /*  1 reserved */
...
static void xembed_request_focus( Display *display, Window window, DWORD timestamp )
{
    ...
    xev.xclient.message_type = broadwaydrv_atom(_XEMBED);
...
static enum event_merge_action merge_events( XEvent *prev, XEvent *next )
{
    ...
    case MotionNotify:
        ...
        case GenericEvent:
            ...
            if (broadwaydrv_thread_data()->warp_serial) break;
            return MERGE_KEEP;
        }
        break;
    case GenericEvent:
 ...
 */
static void set_input_focus( struct broadwaydrv_win_data *data )
{
...
static void handle_manager_message( HWND hwnd, XClientMessageEvent *event )
{
    ...
    if (systray_atom && event->data.l[1] == systray_atom)
    {
        ...
        broadwaydrv_client_func( client_func_systray_change_owner, &params, sizeof(params) );
    }
}
...
static void handle_wm_protocols( HWND hwnd, XClientMessageEvent *event )
{
    ...
    if (protocol == broadwaydrv_atom(WM_DELETE_WINDOW))
    {
    ...
    else if (protocol == broadwaydrv_atom(WM_TAKE_FOCUS))
    {
        HWND last_focus = broadwaydrv_thread_data()->last_focus;
    ...
    else if (protocol == broadwaydrv_atom(_NET_WM_PING))
    {
...
static BOOL BROADWAYDRV_FocusIn( HWND hwnd, XEvent *xev )
{
    ...
    if (!can_activate_window(hwnd))
    {
        ...
        if (hwnd) hwnd = NtUserGetAncestor( hwnd, GA_ROOT );
        if (!hwnd) hwnd = get_active_window();
        if (!hwnd) hwnd = broadwaydrv_thread_data()->last_focus;
...
static void focus_out( Display *display , HWND hwnd )
 {
    ...
    broadwaydrv_thread_data()->last_focus = hwnd;
...
static BOOL BROADWAYDRV_FocusOut( HWND hwnd, XEvent *xev )
{
    ...
    {
        if (!hwnd && event->window == broadwaydrv_thread_data()->clip_window) NtUserClipCursor( NULL );
        return TRUE;
    }
    if (!hwnd) return FALSE;
...
static BOOL BROADWAYDRV_MapNotify( HWND hwnd, XEvent *event )
{
    ...
    if (event->xany.window == broadwaydrv_thread_data()->clip_window) return TRUE;
...
static int get_window_wm_state( Display *display, Window window )
{
    ...
    if (!XGetWindowProperty( display, window, broadwaydrv_atom(WM_STATE), 0,
                             sizeof(*state)/sizeof(CARD32), False, broadwaydrv_atom(WM_STATE),
                             &type, &format, &count, &remaining, (unsigned char **)&state ))
    {
        if (type == broadwaydrv_atom(WM_STATE) && get_property_size( format, count ) >= sizeof(*state))
            ret = state->state;
        XFree( state );
    }
    return ret;
...
static BOOL BROADWAYDRV_PropertyNotify( HWND hwnd, XEvent *xev )
{
    ...
    if (!hwnd) return FALSE;
    if (event->atom == broadwaydrv_atom(WM_STATE)) handle_wm_state_notify( hwnd, event, TRUE );
    return TRUE;
}
...
static Bool is_wm_state_notify( Display *display, XEvent *event, XPointer arg )
{
    ...
    return (event->type == DestroyNotify ||
            (event->type == PropertyNotify && event->xproperty.atom == broadwaydrv_atom(WM_STATE)));
}
...
static void post_drop( HWND hwnd, DROPFILES *drop, ULONG size )
{
    ...
    broadwaydrv_client_func( client_func_dnd_post_drop, drop, size );
}
...
static void EVENT_DropFromOffiX( HWND hWnd, XClientMessageEvent *event )
{
    ...
    XGetWindowProperty( event->display, DefaultRootWindow(event->display),
                        broadwaydrv_atom(DndSelection), 0, 65535, FALSE,
...
{
  struct broadwaydrv_win_data *win_data;
  unsigned long	data_length;
  unsigned long	aux_long;
...
static void handle_xdnd_enter_event( HWND hWnd, XClientMessageEvent *event )
{
    ...
    if (event->data.l[1] & 1)
    {
        ...
        XGetWindowProperty( event->display, event->data.l[0], broadwaydrv_atom(XdndTypeList),
    ...
    data = import_xdnd_selection( event->display, event->window, broadwaydrv_atom(XdndSelection),
                                  xdndtypes, count, &size );
    if (data)
    {
        broadwaydrv_client_func( client_func_dnd_enter_event, data, size );
        free( data );
    }
...
static DWORD xdnd_action_to_drop_effect( long action )
{
    ...
    if (action == broadwaydrv_atom(XdndActionCopy))
        return DROPEFFECT_COPY;
    else if (action == broadwaydrv_atom(XdndActionMove))
        return DROPEFFECT_MOVE | DROPEFFECT_COPY;
    else if (action == broadwaydrv_atom(XdndActionLink))
        return DROPEFFECT_LINK | DROPEFFECT_COPY;
    else if (action == broadwaydrv_atom(XdndActionAsk))
...
static long drop_effect_to_xdnd_action( UINT effect )
{
    if (effect == DROPEFFECT_COPY)
        return broadwaydrv_atom(XdndActionCopy);
    else if (effect == DROPEFFECT_MOVE)
        return broadwaydrv_atom(XdndActionMove);
    else if (effect == DROPEFFECT_LINK)
        return broadwaydrv_atom(XdndActionLink);
...
static void handle_xdnd_position_event( HWND hwnd, XClientMessageEvent *event )
{
    ...
    effect = broadwaydrv_client_func( client_func_dnd_position_event, &params, sizeof(params) );
    ...
    e.message_type = broadwaydrv_atom(XdndStatus);
    e.format = 32;
...
static void handle_xdnd_drop_event( HWND hwnd, XClientMessageEvent *event )
{
    ...
    effect = broadwaydrv_client_call( client_dnd_drop_event, HandleToUlong( hwnd ));
    ...
    e.display = event->display;
    e.window = event->data.l[0];
    e.message_type = broadwaydrv_atom(XdndFinished);
    e.format = 32;
    e.data.l[0] = event->window;
    e.data.l[1] = !!effect;
...
{
    broadwaydrv_client_call( client_dnd_leave_event, 0 );
}
