...
static struct broadwaydrv_display_device_handler host_handler;
static struct broadwaydrv_settings_handler settings_handler;

#define NEXT_DEVMODEW(mode) ((DEVMODEW *)((char *)((mode) + 1) + (mode)->dmDriverExtra))

struct broadwaydrv_display_depth
{
    struct list entry;
    broadwaydrv_settings_id display_id;
    DWORD depth;
};

/* Display device emulated depth list, protected by modes_section */
static struct list broadwaydrv_display_depth_list = LIST_INIT(broadwaydrv_display_depth_list);
 ...
 *
 */
static BOOL nores_get_id(const WCHAR *device_name, BOOL is_primary, broadwaydrv_settings_id *id)
{
    id->id = is_primary ? 1 : 0;
    return TRUE;
}

static BOOL nores_get_modes(broadwaydrv_settings_id id, DWORD flags, DEVMODEW **new_modes, UINT *mode_count)
{
...
static BOOL nores_get_current_mode(broadwaydrv_settings_id id, DEVMODEW *mode)
{
    RECT primary = get_host_primary_monitor_rect();
...
static LONG nores_set_current_mode(broadwaydrv_settings_id id, const DEVMODEW *mode)
{
...
{
    struct broadwaydrv_display_depth *display_depth;

    pthread_mutex_lock( &settings_mutex );
    LIST_FOR_EACH_ENTRY(display_depth, &broadwaydrv_display_depth_list, struct broadwaydrv_display_depth, entry)
    {
    ...
    display_depth->depth = depth;
    list_add_head(&broadwaydrv_display_depth_list, &display_depth->entry);
    pthread_mutex_unlock( &settings_mutex );
}

static DWORD get_display_depth(broadwaydrv_settings_id display_id)
{
    struct broadwaydrv_display_depth *display_depth;
    DWORD depth;
...
static DEVMODEW *get_full_mode(broadwaydrv_settings_id id, DEVMODEW *dev_mode)
{
...
static LONG apply_display_settings( DEVMODEW *displays, broadwaydrv_settings_id *ids, BOOL do_attach )
{
    DEVMODEW *full_mode;
    BOOL attached_mode;
    LONG count, ret;
    ...
    {
        broadwaydrv_settings_id *id = ids + count;
...
RECT get_work_area(const RECT *monitor_rect)
{
    ...
    if (!XGetWindowProperty(gdi_display, DefaultRootWindow(gdi_display),
                            broadwaydrv_atom(_GTK_WORKAREAS_D0), 0, ~0, False, XA_CARDINAL, &type,
    ...
    if (!XGetWindowProperty(gdi_display, DefaultRootWindow(gdi_display), broadwaydrv_atom(_NET_WORKAREA),
...
BOOL BROADWAYDRV_UpdateDisplayDevices( const struct gdi_device_manager *device_manager, BOOL force, void *param )
{
    ...
    for (gpu = 0; gpu < gpu_count; gpu++)
    {
        ...
        for (adapter = 0; adapter < adapter_count; adapter++)
        {
            ...
            WCHAR devname[32];
            char buffer[32];
            broadwaydrv_settings_id settings_id;
