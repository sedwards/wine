...
static XIM xim_create( struct broadwaydrv_thread_data *data )
{
...
static void xim_open( Display *display, XPointer user, XPointer arg )
{
    struct broadwaydrv_thread_data *data = (void *)user;
...
void xim_thread_attach( struct broadwaydrv_thread_data *data )
{
...
static XIC xic_create( XIM xim, HWND hwnd, Window win )
{
    ...
    XIC xic;
    XFontSet fontSet = broadwaydrv_thread_data()->font_set;
...
XIC X11DRV_get_ic( HWND hwnd )
{
    ...
    broadwaydrv_thread_data()->last_xic_hwnd = hwnd;
    if (!(ret = data->xic) && (xim = broadwaydrv_thread_data()->xim))
