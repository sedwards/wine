 ...
 *              update_blit_data
 */
static void update_blit_data(struct broadwaydrv_window_surface *surface)
{
 ...
 */
static void *broadwaydrv_surface_get_bitmap_info(struct window_surface *window_surface,
                                            BITMAPINFO *info)
{
    struct broadwaydrv_window_surface *surface = get_broadway_surface(window_surface);
...
UINT broadwaydrv_RealizePaletteEntries(DWORD num_entries, PALETTEENTRY* entries);
...
struct window_surface *create_surface(HWND window, const RECT *rect, struct window_surface *old_surface, BOOL use_alpha)
{
    ...
    struct broadwaydrv_window_surface *old_broadway_surface = get_broadway_surface(old_surface);
    int width = rect->right - rect->left, height = rect->bottom - rect->top;
    pthread_mutexattr_t attr;
    int err;

    surface = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
                        FIELD_OFFSET(struct broadwaydrv_window_surface, info.bmiColors[256]));
