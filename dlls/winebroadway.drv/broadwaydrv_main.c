...
static broadwaydrv_error_callback err_callback;   /* current callback for error */
...
static void set_queue_display_fd( Display *display )
{
    ...
    {
        MESSAGE( "broadwaydrv: Can't allocate handle for display fd\n" );
    ...
    if (ret)
    {
        MESSAGE( "broadwaydrv: Can't store handle for display fd\n" );
        NtTerminateProcess( 0, 1 );
    }
    NtClose( handle );
}
...
struct broadwaydrv_thread_data *broadwaydrv_init_thread_data(void)
{
    ...
    {
        ERR_(winediag)( "broadwaydrv: Can't open display: %s. Please ensure that your X server is running and that $DISPLAY is set correctly.\n", XDisplayName(NULL));
        NtTerminateProcess( 0, 1 );
    }
...
{
    broadwaydrv_init,
    broadwaydrv_systray_clear,
    broadwaydrv_systray_dock,
    broadwaydrv_systray_hide,
    broadwaydrv_systray_init,
    broadwaydrv_tablet_attach_queue,
    broadwaydrv_tablet_get_packet,
    broadwaydrv_tablet_info,
    broadwaydrv_tablet_load_info,
};
...
static NTSTATUS broadwaydrv_wow64_systray_clear( void *arg )
{
    HWND hwnd = UlongToPtr( *(ULONG *)arg );
    return broadwaydrv_systray_clear( &hwnd );
}

static NTSTATUS broadwaydrv_wow64_systray_dock( void *arg )
{
    struct
    {
        UINT64 event_handle;
    ...
    return broadwaydrv_systray_dock( &params );
}

static NTSTATUS broadwaydrv_wow64_systray_hide( void *arg )
{
    HWND hwnd = UlongToPtr( *(ULONG *)arg );
    return broadwaydrv_systray_hide( &hwnd );
}

static NTSTATUS broadwaydrv_wow64_tablet_get_packet( void *arg )
{
    FIXME( "%p\n", arg );
    return 0;
}

static NTSTATUS broadwaydrv_wow64_tablet_info( void *arg )
{
    struct
    {
        UINT category;
    ...
    return broadwaydrv_tablet_info( &params );
}

const unixlib_entry_t __wine_unix_call_wow64_funcs[] =
{
    broadwaydrv_wow64_init,
    broadwaydrv_wow64_systray_clear,
    broadwaydrv_wow64_systray_dock,
    broadwaydrv_wow64_systray_hide,
    broadwaydrv_systray_init,
    broadwaydrv_tablet_attach_queue,
    broadwaydrv_wow64_tablet_get_packet,
    broadwaydrv_wow64_tablet_info,
    broadwaydrv_tablet_load_info,
};
