...
C_ASSERT(NUM_EVENT_TYPES <= sizeof(rdsdrv_event_mask) * 8);
 ...
 *              setup_options
 *
 * Set up the RDS driver options.
 */
static void setup_options(void)
{
    static const WCHAR rdsdriverW[] = {'\\','R','D','S',' ','D','r','i','v','e','r',0};
    WCHAR buffer[MAX_PATH + 16], *p, *appname;
    HKEY hkey, appkey = 0;
    DWORD len;

    /* @@ Wine registry key: HKCU\Software\Wine\RDS Driver */
    hkey = open_hkcu_key("Software\\Wine\\RDS Driver");
    ...
    if (len && len < MAX_PATH)
    {
        ...
        memcpy(buffer + len, rdsdriverW, sizeof(rdsdriverW));
        /* @@ Wine registry key: HKCU\Software\Wine\AppDefaults\app.exe\RDS Driver */
 ...
 *              rdsdrv_init
 */
static NTSTATUS rdsdrv_init(void *arg)
{
    ...
    rdsdrv_err_on = ERR_ON(rdsdrv);
    if (rdsdrv_start_cocoa_app(NtGetTickCount()))
    {
 ...
 *              ThreadDetach   (MACDRV.@)
 */
void rdsdrv_ThreadDetach(void)
{
    struct rdsdrv_thread_data *data = rdsdrv_thread_data();

    if (data)
    {
        rdsdrv_destroy_event_queue(data->queue);
...
static void set_queue_display_fd(int fd)
{
    ...
    {
        MESSAGE("rdsdrv: Can't allocate handle for event queue fd\n");
    ...
    if (ret)
    {
        MESSAGE("rdsdrv: Can't store handle for event queue fd\n");
        NtTerminateProcess(0, 1);
    }
    NtClose(handle);
}
...
/***********************************************************************
 *              rdsdrv_init_thread_data
 */
struct rdsdrv_thread_data *rdsdrv_init_thread_data(void)
{
    struct rdsdrv_thread_data *data = rdsdrv_thread_data();
    ...
    if (!(data->queue = rdsdrv_create_event_queue(rdsdrv_handle_event)))
    {
        ERR("rdsdrv: Can't create event queue.\n");
        NtTerminateProcess(0, 1);
    }

    rdsdrv_get_input_source_info(&data->keyboard_layout_uchr, &data->keyboard_type, &data->iso_keyboard, &input_source);
    data->active_keyboard_layout = rdsdrv_get_hkl_from_source(input_source);
    CFRelease(input_source);
    rdsdrv_compute_keyboard_layout(data);

    set_queue_display_fd(rdsdrv_get_event_queue_fd(data->queue));
 ...
 */
BOOL rdsdrv_SystemParametersInfo( UINT action, UINT int_param, void *ptr_param, UINT flags )
{
    switch (action)
    {
...
static NTSTATUS rdsdrv_quit_result(void *arg)
{
    struct quit_result_params *params = arg;
    rdsdrv_quit_reply(params->result);
    return 0;
}
